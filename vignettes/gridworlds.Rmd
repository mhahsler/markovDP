...
gridworld_matrix(x, what = "unreachable")
```
...
gridworld_plot(sol, zlim = c(0, 2), sub = "Iteration 5")
```
...
```{r, fig.show='animate'}
gridworld_animate(x, "value_iteration", n = 15, zlim = c(0, 2))
```
...
```{r, fig.show='animate'}
gridworld_animate(x, "policy_iteration", n = 15, zlim = c(0, 2))
```

```{r, fig.show='animate'}
gridworld_animate(x, "q_learning", n = 15, zlim = c(0, 2), horizon = 100)
```
