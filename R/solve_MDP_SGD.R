...
solve_MDP_SGD <-
  function(model,
    ...
    if (is.null(q$w_init) || is.null(q$f) || is.null(q$gradient))
      stop("Approx q-function f, the gradient, or w_init are missing in the q_function element in the model!")
...
#' @rdname solve_MDP_SGD
#' @export
add_linear_approx_Q_function <- function(model, state_features = NULL) {
  
  if (is.null(state_features)) { 
    if (!is.null(model$info$gridworld)) {
      # for grid worlds we use the x/y coordinates in the maze as the state feature vectors
      state_features <- t(gw_s2rc(S(model)))
      colnames(state_features) <- c("x", "y")
    } else {
      stop("state_features are missing!")
    }
  }
  
  if (!is.matrix(state_features) || !nrow(state_features) == c(length(S(model))))
    stop("state_features needs to be a matrix with one row per state!")
  rownames(state_features) <- S(model)
  
  A <- A(model)
  
  # Construct state-action feature vectors
  X <- lapply(seq_along(A), FUN = function(a) { 
      x <- matrix(0, nrow = nrow(state_features), ncol = 1L + length(A) * ncol(state_features),
...
#' @export
approx_Q_value <- function(model, state = NULL, action = NULL, w = NULL) {
  state <- state %||% S(model)
  if(length(state) > 1L)
    return(t(sapply(state, FUN = function(s) approx_Q_value(model, s, action, w))))
  
  action <- action %||% A(model)
  if(length(action) > 1L)
    return(sapply(action, FUN = function(a) approx_Q_value(model, state, a, w)))
...
#' @rdname solve_MDP_SGD
#' @export
approx_greedy_action <- function(model, s, w = NULL, epsilon = 0) {
  ...
  q <- model$approx_Q_function
  qs <- sapply(seq_along(A(model)), FUN = function(a) q$f(s, a, w))
...
approx_greedy_policy <- function(model, w = NULL) {
  w <- w %||% model$solution$w
  acts <- sapply(S(model), FUN = function(s) approx_greedy_action(model, s, w, epsilon = 0))
  
  data.frame(
    state = S(model),
    V = sapply(seq_along(S(model)), FUN = function(i) approx_Q_value(model, i, acts[i], w)),
    action = acts,
    row.names = NULL
  )
}
